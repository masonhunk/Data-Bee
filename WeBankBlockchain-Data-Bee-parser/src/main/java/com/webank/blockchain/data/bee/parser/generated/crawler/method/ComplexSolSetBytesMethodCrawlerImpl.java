...
import com.webank.blockchain.data.bee.parser.generated.bo.method.ComplexSolSetBytesBO;

@Slf4j
@Service
@ConditionalOnProperty(name = "monitor.ComplexSol.setBytesMethodCrawlerService", havingValue = "on")
public class ComplexSolSetBytesMethodCrawlerImpl implements BcosMethodCrawlerInterface {
	...
	public MethodBO transactionHandler(JsonTransactionResponse transaction, TransactionReceipt receipt, Date blockTimeStamp,
	  String methodName) {
		log.debug("Begin process ComplexSolSetBytes Transaction");
		ComplexSolSetBytesBO entity = new ComplexSolSetBytesBO();
		...
		entity.setIdentifier("ComplexSolSetBytes");	
		...
		try {			
		    ...
		    if (!CollectionUtils.isEmpty(params)) {		
				entity.setB(BytesUtils.bytesArrayToString(params.get(0)));
		}
			if (receipt.getOutput().length() > 2) {		
				...
				entity.setOutput1(BytesUtils.bytesArrayToString(outputList.get(0)));
				}  
        	}
		...
		log.debug("end process ComplexSolSetBytes Transaction");
		return entity;
	}
}
