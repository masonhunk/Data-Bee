...
import com.webank.blockchain.wecredit.contracts.ComplexSol.LogSetBytesEventResponse;
import com.webank.blockchain.data.bee.parser.crawler.face.BcosEventCrawlerInterface;
import com.webank.blockchain.data.bee.parser.generated.bo.event.ComplexSolLogSetBytesBO;
...
@Service
@Slf4j
@ConditionalOnProperty(name = "monitor.ComplexSol.LogSetBytesCrawlerService", havingValue = "on")
public class ComplexSolLogSetBytesCrawlerImpl implements BcosEventCrawlerInterface {
	@Autowired
	...
	public List<EventBO> handleReceipt(TransactionReceipt receipt, Date blockTimeStamp) {
        ...
        try {
            	Map<String, List<Object>> map = decoder.decodeEvents(contract.ABI, receipt.getLogs());
           		List<Object> events = map.get(contract.LOGSETBYTES_EVENT.getName());
           		
                if (!CollectionUtils.isEmpty(events)) {             
                        ComplexSolLogSetBytesBO complexSolLogSetBytes = new ComplexSolLogSetBytesBO();                
                        complexSolLogSetBytes.setIdentifier("ComplexSolLogSetBytes");		
						complexSolLogSetBytes.setBlockHeight(receipt.getBlockNumber());
						complexSolLogSetBytes.setEventContractAddress(receipt.getContractAddress());
						complexSolLogSetBytes.setTxHash(receipt.getTransactionHash());				
						
							complexSolLogSetBytes.setO(String.valueOf(events.get(0)));
							complexSolLogSetBytes.setB(String.valueOf(events.get(1)));
							complexSolLogSetBytes.setBlockTimeStamp(blockTimeStamp);
							log.debug("depot LogSetBytes:{}", complexSolLogSetBytes.toString());
							list.add(complexSolLogSetBytes);						
                }
        } catch (Exception e) {
