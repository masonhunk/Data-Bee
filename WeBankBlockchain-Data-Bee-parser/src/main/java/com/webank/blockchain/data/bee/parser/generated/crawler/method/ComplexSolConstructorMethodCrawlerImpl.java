...
import com.webank.blockchain.data.bee.parser.generated.bo.method.ComplexSolConstructorBO;

@Slf4j
@Service
@ConditionalOnProperty(name = "monitor.ComplexSol.constructorMethodCrawlerService", havingValue = "on")
public class ComplexSolConstructorMethodCrawlerImpl implements BcosMethodCrawlerInterface {
	...
	public MethodBO transactionHandler(JsonTransactionResponse transaction, TransactionReceipt receipt, Date blockTimeStamp,
	  String methodName) {
		log.debug("Begin process ComplexSolConstructor Transaction");
		ComplexSolConstructorBO entity = new ComplexSolConstructorBO();
		...
		entity.setIdentifier("ComplexSolConstructor");	
		...
		try {			
		    ...
		    if (!CollectionUtils.isEmpty(params)) {		
				...
				entity.setS(String.valueOf(params.get(1)));
		}
		...
		log.debug("end process ComplexSolConstructor Transaction");
		return entity;
	}
}
